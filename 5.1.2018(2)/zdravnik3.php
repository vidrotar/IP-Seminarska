...
<head>
  ...
  <title>Rezultati(zdravnik)</title>
</head>
<?php 
...
<body">
    ...
    <?php include 'povezava.php';?> <!-- povezava z bazo -->
      ...
      <?php 
        ...
        $datum = $_POST["datum"];
        // MAX DATUM
        pisiConsola("DATUM " . $datum);
        foreach ($_POST as $key => $value) {
          $i = $i +1;
          if($i > 4 ){
            $sql = "SELECT vprasanje.id_vprasanje, odgovor.id_odgovor FROM vprasanje, odgovor where odgovor.id_vprasanje=vprasanje.id_vprasanje and odgovor.id_odgovor=$value";
            $rez = $conn->query($sql);
            if ($rez != null && $rez->num_rows > 0) {
              while($row = $rez->fetch_assoc()) {
                $idVpras = $row["id_vprasanje"];
                $idOdgov = $row["id_odgovor"];
                pisiConsola("id_vprasanje:" . $idVpras . " id_odgovor:" . $idOdgov);
                $update = "UPDATE vpr_odg SET id_odgovor = ".$idOdgov." WHERE vpr_odg.id_vprasanje=" .$idVpras." and vpr_odg.emso=".$emso." and vpr_odg.datum >=".$datum."";
                
                if ($conn->query($update) === TRUE) {
                  pisiConsola("odgovor update");
                } else {
                    pisiConsola( "Error: " . $sql . "<br>" . $conn->error);
                }
              }
            }                  
          }
